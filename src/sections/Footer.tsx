          ...
          <nav className="flex flex-col md:flex-row items-center gap-8">
            {footerLinks.map((link) => (
              <a
                ...
                className="inline-flex items-center gap-1.5 text-white hover:text-cyan-400 transition-all duration-300 transform hover:scale-125"
              >
                <span className="font-semibold">{link.title}</span>
                <ArrowImg className="size-4 transform transition-all duration-300 hover:scale-110" />
              </a>
            ))}
          </nav>
        </div>
